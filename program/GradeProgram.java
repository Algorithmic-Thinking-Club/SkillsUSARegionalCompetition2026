...
public class GradeProgram {
    private Map<String, List<Double>> gradesByClass = new HashMap<>(); // creating a hashmap to store table of values
    ...
    private void createGUI() {
        JFrame frame = new JFrame("[#] - Program 2 - Grade Tracker"); // just creating the frame
        ...
        calcButton.addActionListener(e -> { // this is the actual function for the calc button
            try {
                ...
                outputArea.setText("oops: " + ex.getMessage()); // exception, if the inputs just say that error
            }
        });
    ...
    private void updateDisplay(String className, JTextArea outputArea) { // this is where outputArea gets updated
        List<Double> grades = gradesByClass.get(className); // list of the grades
        if (grades.isEmpty()) {
            outputArea.setText("no grades for " + className + " yet"); // if they are empty throw this error
            return;
        }
    ...
    private double parseGrade(JTextField field) { // parsing the grade for safety and returning the grade
        try {
            ...
            if (text.isEmpty()) throw new Exception("grade cant be empty"); // ensure its not empty
            double grade = Double.parseDouble(text);
            if (grade < 0 || grade > 100) throw new Exception("grade has to be 0-100"); // confining value
